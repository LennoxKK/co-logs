# =====================================================================
# debug_logger_pull.ps1
# =====================================================================
# <DESCRIPTION>
# Tool: Android Debug Logger Pull Tool
# Version: 1.0
# Author: LKK
# Date: April 7, 2025
# Purpose: Pull debug log data from Android device for analysis
# Requirements: ADB (Android Debug Bridge), PowerShell 5.0+
# Usage: ./debug_logger_pull.ps1 [destination_directory]
# Features: 
#   - Automated extraction of device debug logs
...
$Host.UI.RawUI.WindowTitle = "Android Debug Logger Pull Tool"
Clear-Host
Write-Host " Android Debug Logger Pull Tool " -ForegroundColor White -BackgroundColor DarkBlue
Write-Host ""
...
# Pull debug logs
Write-Host " > Pulling debug logs from device..." -ForegroundColor Yellow
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$outputPath = Join-Path $DestinationDir "debuglogger_$timestamp"

try {
    # First attempt regular pull
    $result = adb pull data/debuglogger "$outputPath" 2>&1
        ...
        if ($filesSkipped -gt 0) {
            ...
            try {
                ...
                if ($rootCheck -match "OK") {
                    ...
                    adb shell "su -c 'mkdir -p /sdcard/temp_debuglogs && cp -r data/debuglogger/* /sdcard/temp_debuglogs/'" 2>&1 | Out-Null
                    
                    # Pull from accessible location
                    $rootResult = adb pull /sdcard/temp_debuglogs "$outputPath-root" 2>&1
                    
                    # Clean up
                    adb shell "su -c 'rm -rf /sdcard/temp_debuglogs'" 2>&1 | Out-Null
    ...
    Write-Host " > ERROR: Failed to pull debug logs" -ForegroundColor Red
